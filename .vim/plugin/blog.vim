...
enable_tags = 1
blog_username = 'admin'
blog_password = open('/home/luke/wordpress-admin-password.txt').read().strip()
...
def blog_list_pages():
  try:
    allpages = handler.getPages('',blog_username, blog_password)
    del vim.current.buffer[:]
    vim.command("set syntax=blogsyntax")
    vim.current.buffer[0] = "\"====== List of Pages ========="
    for p in allposts:
      vim.current.buffer.append(("".zfill(size-len(p["postid"])).replace("0", " ")+p["postid"])+"\t"+(p["title"]).encode("utf-8"))
      vim.command('set nomodified')
    blog_edit_off()
